  ...
  void modelContext.registerTool(
    {
      ...
      inputSchema: {
        ...
        properties: {
          param: {
            type: 'string',
            enum: Object.keys(samplerParams),
            description: Object.entries(samplerParams)
      ...
      execute: ({ param, value }) => {
        if (!(param in samplerParams)) throw new Error(`Unknown param "${param}".`);
        const key = param as SamplerParamKey;
        const { min, max } = samplerParams[key];
